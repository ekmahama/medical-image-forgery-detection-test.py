...
class ELM(object):
    ...
    def setWeights(self):
        ...
        weights[0] = np.random.random_sample(
...
class MultiClassify(ELM):
    ...
    def buildNetwork(self, input_dim):
        ...
        # Input layer
        self.model.add(Dense(units=100, activation=self.input_layer,
                             input_dim=input_dim, trainable=False))
        self.setWeights()
        # Hidden layer
        ...
        # Output layer
        self.model.add(
            Dense(units=3, activation=self.output_layer, trainable=True))
...
class BinaryClassify(ELM):
    ...
    def buildNetwork(self, input_dim):
        ...
        # Input layer
        self.model.add(Dense(units=110, activation=self.input_layer,
                             input_dim=input_dim, trainable=False))
        self.setWeights()
        # Hidden layer
        for act in self.hidden_layer:
        ...
        # Output layer
        self.model.add(
            Dense(units=2, activation=self.output_layer, trainable=True))
